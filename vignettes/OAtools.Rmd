...
With the filepath specified, we use the `excelToSummarizedExperiment()` 
...
loaded in using `excelToSummarizedExperiment()`. Note that it does not 
...
Then, to convert the SummarizedExperiment container into a *NormqPCR*-friendly instance of the qPCRBatch class, simply invoke the `seToQPCRBatch()` function

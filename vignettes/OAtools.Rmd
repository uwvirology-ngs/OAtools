...
reaction on the OpenArray plate. OAtools exports the `computeModels()`
...
parameters of best fit and appends them to the metadata of the 
SummarizedExperiment. 

Under the hood, `computeModels()` invokes the `runFitCurve`()` function, 
...
Among the features appended by `computeModels()` are the slope at the 
...
derive PCR results. We can invoke the `determinePCRResults()` function on our
SummarizedExperiment to do this programmatically. `determinePCRResults()` 
...
The outcome of running the `determinePCRResults()` function is to add columns 

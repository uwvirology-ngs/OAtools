...
lab uses to screen patient swabs for respiratory viral infection. With 3000+ 
...
Relative Quantification Application for the data analysis. However, to our 
knowledge, no all-in-one open-source software exists to derive cycle thresholds
and sample results from raw fluroescence curves measured on the OpenArray 
platform. To bridge the gap, OAtools streamlines the analysis process 
including data import, quality control plots, analysis, and communication
through an R Markdown document. Our goal in sharing OAtools is to support 
open-source and shareable analyses of OpenArray gene expression data, 
especially in clinical research environments.
...
variable as below. The excel_to_summarized_experiment() function transforms 
the raw data output in .xlsx format from QuantStudio 12K Flex software into a 
SummarizedExperiment class object, which is the intended input for downstream 
functions. As there are 40 cycles in each unique qPCR reaction, the rowData 
...
run_data <- as.data.frame(SummarizedExperiment::colData(se))

# print a slice of the SummarizedExperiment
run_data |> 
    dplyr::select(well, sample_name, target_name, crt, amp_score:amp_status) |> 
    dplyr::slice(1:10) |> 
